...
class SmartConsole {
  ...
  static setup(options) {
    if (!SmartConsole.$instance) {
      const opts = { 
        ...options,
        prefix: (level) => {
          return `[${SmartConsole.timeString()}]`;
        } 
      };
      SmartConsole.$instance = new SmartConsole(opts);
      window.log = SmartConsole.instance;
    }

    return SmartConsole.$instance;
  }

  static get instance() {
    return SmartConsole.setup();
  }
